...
```
[대기] tim2_HCSR04_Start == 'n'
  └─ OpCnt > 125,000 → TRIG HIGH, Start = 'y'

[TRIG HIGH] tim2_HCSR04_TrigHigh == 'y'
  └─ 즉시 TRIG HIGH 출력 → TrigLow = 'y'

[TRIG LOW] tim2_HCSR04_TrigLow == 'y'
  └─ OpCnt > 25 → TRIG LOW 출력 → EchoState = 'y'

[Echo 계측] tim2_HCSR04_EchoState == 'y'
  ├─ EchoSample 업데이트 (4비트 시프트 레지스터)
  ├─ 0x0F → EchoCnt 증가
  ├─ 0x0C → Falling 감지 → 거리 계산
  └─ OpCnt > 7,500 → 타임아웃, 재시작
```
